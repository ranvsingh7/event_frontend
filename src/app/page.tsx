...
try {
    ...
    console.error("Error decoding token:", error);
        ...
        const fetchEvents = async () => {
            try {
                ...
                const filterData= data.filter((event) => event.createdBy._id === userId);
    ...
    const addEntryType = () => {
        if (editEvent) {
            setEditEvent({
                ...editEvent,
                entryTypes: [...editEvent.entryTypes, { name: "", amount: 0, count: 0 }],
            });
        }
    };
    ...
    const removeEntryType = (index: number) => {
        if (editEvent) {
            const updatedEntryTypes = editEvent.entryTypes.filter((_, i) => i !== index);
            setEditEvent({ ...editEvent, entryTypes: updatedEntryTypes });
        }
    };
